...
test_file_post_image = os.path.join(script_dir, 'dummy_test_pic_post.jpg')
test_file_post_image_two = os.path.join(script_dir, 'dummy_test_pic_post_two.jpg')
test_file_post_video = os.path.join(script_dir, 'dummy_test_video_post.mp4')
test_file_post_audio = os.path.join(script_dir, 'dummy_test_audio_post.mp3')
...
class TestApi(TestCase):
    ...
    def test_M_posts_get_user_posts(self):
        self.response = self.session.get(f"{settings.BASE_URL}/api/posts/getUserPosts/?user_id={test_user_one['user_id']}&limit=5")
        response_body = self.response.json()
        self.assertEqual(self.response.status_code, HTTP_STATUS["OK"])
        ...
        self.assertTrue(response_body['success'])
        # Check audio post
        self.assertEqual(response_body['posts'][0]['text_content'], test_post_audio["text_content"])
        ...
        # check video post
        self.assertEqual(response_body['posts'][1]['text_content'], test_post_video["text_content"])
        ...
        # check image post
        self.assertEqual(response_body['posts'][2]['text_content'], test_post_image["text_content"])
        ...
        # check text post
        self.assertEqual(response_body['posts'][3]['text_content'], test_post_text["text_content"])
    ...
    def test_N_posts_get_single_post(self):
        self.response = self.session.get(f"{settings.BASE_URL}/api/posts/getSinglePost/{test_post_video['post_id']}/")
        response_body = self.response.json()
        self.assertEqual(self.response.status_code, HTTP_STATUS["OK"])
        self.assertTrue(response_body['success'])
        self.assertEqual(response_body['post']['text_content'], test_post_video["text_content"])
        self.assertEqual(response_body['post']['hashtags'], test_hashtags)
        self.assertEqual(response_body['post']['post_type'], "video")
        self.assertEqual(response_body['post']['file_url'], test_post_video["file_url"])

    def test_O_get_multiple_posts(self):
        self.response = self.session.post(f"{settings.BASE_URL}/api/posts/getMultiplePosts/",
                                        data=json.dumps({
    ...
    def test_P_edit_post(self):
        ...
        self.response_edit = self.session.put(f"{settings.BASE_URL}/api/posts/editPost/",
                                        data=json.dumps({
        ...
        self.assertEqual(self.response_edit.status_code, HTTP_STATUS["Created"])
        self.assertTrue(response_body_edit['success'])
        self.assertEqual(response_body_edit['message'], "Post edited successfully.")
        # Retrieve the post and check if its edited.
        self.response_get = self.session.get(f"{settings.BASE_URL}/api/posts/getUserPosts/?user_id={test_user_one['user_id']}&limit=5")
        response_body_get = self.response_get.json()
        self.assertEqual(self.response.status_code, HTTP_STATUS["OK"])
        self.assertEqual(response_body_get['posts'][2]['text_content'], new_text_content)
        self.assertEqual(response_body_get['posts'][2]['hashtags'], new_hashtags)
        #self.assertEqual(response_body['posts'][2]['file_url'], )
        self.assertEqual(response_body_get['posts'][2]['post_type'], "image")
    
    def test_Q_delete_post(self):
        self.response = self.session.delete(f"{settings.BASE_URL}/api/posts/deletePost/{test_post_image['post_id']}/")
        response_body = self.response.json()
        self.assertEqual(self.response.status_code, HTTP_STATUS["OK"])
        self.assertTrue(response_body['success'])
        self.assertEqual(response_body['message'], "Post deleted successfully.")
    ...
    def test_R_delete_post_already_deleted(self):
        ...
        self.assertTrue(response_body['success'])
        self.assertEqual(response_body['message'], "Post does not exist or already deleted.")
    
    def test_S_posts_get_single_post_not_exist(self):
        self.response = self.session.get(f"{settings.BASE_URL}/api/posts/getSinglePost/{test_post_image['post_id']}/")
        response_body = self.response.json()
    ...
    def test_T_posts_add_bookmark(self):
        self.response = self.session.post(f"{settings.BASE_URL}/api/posts/addPostBookmark/",
                                        data=json.dumps({
        ...
        self.assertEqual(self.response.status_code, HTTP_STATUS["Created"])
        self.assertTrue(response_body['success'])
        self.assertEqual(response_body['message'], "Post bookmarked successfully.")
    ...
    def test_V_posts_get_bookmarks(self):
        self.response = self.session.get(f"{settings.BASE_URL}/api/posts/getBookmarkedPosts/")
        response_body = self.response.json()
        self.assertEqual(self.response.status_code, HTTP_STATUS["OK"])
        self.assertTrue(response_body['success'])
        self.assertEqual(response_body['posts'][0]['post_id'], test_post_text["post_id"])
        self.assertEqual(response_body['posts'][0]['text_content'], test_post_text["text_content"])
        self.assertEqual(response_body['posts'][0]['hashtags'], test_hashtags)
        self.assertEqual(response_body['posts'][0]['post_type'], "text")

    def test_W_posts_delete_bookmark(self):
        self.response = self.session.delete(f"{settings.BASE_URL}/api/posts/deletePostBookmark/{test_post_text['post_id']}/")
        response_body = self.response.json()
        self.assertEqual(self.response.status_code, HTTP_STATUS["OK"])
        self.assertTrue(response_body['success'])
        self.assertEqual(response_body['message'], "Post bookmark deleted successfully.")
