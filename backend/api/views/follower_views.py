...
def follow_user(request):
    ...
    user = request.user
    following, created = Following.objects.get_or_create(user=user, following=user_two)
    if created == False:
...
def get_following(request, user_id):
    ...
    following_users = User.objects.filter(id__in=following).values('id', 'username')
...
def get_followers(request, user_id):
    ...
    followers_users = User.objects.filter(id__in=following).values('id', 'username')
...
def check_is_following(request, user_id):
    ...
    is_following = Following.objects.filter(user=user, following=user_two).exists()
    if is_following:
